...
const fragmentShader = `
  ...
  void main() {
    ...
    float colorOffset = vElevation * 2.0 + 0.5;
    vec3 color = mix(uColor, vec3(1.0), colorOffset * uColorIntensity);

    // Add some shimmer effect
    float shimmer = sin(vUv.x * 10.0 + uTime) * 0.1 + 0.9;

    gl_FragColor = vec4(color * shimmer, 0.8);
  }
`;

export function LiquidMaterial({ color = "#3080ff", intensity = 0.5 }) {
  const { timeOfDay, mode, performanceMode } = useHeroStore();
  ...
  useFrame(({ clock }) => {
    if (materialRef.current) {
      ...
      if (timeOfDay === "night") {
        materialRef.current.uniforms.uColorIntensity.value = 0.8;
      } else if (timeOfDay === "evening") {
        materialRef.current.uniforms.uColorIntensity.value = 0.6;
      } else {
        materialRef.current.uniforms.uColorIntensity.value = intensity;
      }
    }
  });
